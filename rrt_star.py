...
def main():
    ...
    for i in range(numNodes):
        ...
        if noCollision(random_point.getCoord(), near_point.getCoord(), obstacle_points):
            ...
            for j in range(len(nodes)):
                if noCollision(random_point.getCoord(), near_point.getCoord(), obstacle_points) and calculateDistance(nodes[j].getCoord(), new_point.getCoord()) < radius:
                    nearest_point.append(nodes[j])

            minimum_point = near_point
            minimum_cost = new_point.getCost()

            for j in range(len(nearest_point)):
                if nearest_point[j].getCost() + calculateDistance(nearest_point[j].getCoord(), new_point.getCoord()) < minimum_cost:
                    minimum_point = nearest_point[j]
                    minimum_cost = nearest_point[j].getCost() + calculateDistance(nearest_point[j].getCoord(), new_point.getCoord())
